...
<v-data-table
  ...
  >
    <template v-slot:top>
      <v-toolbar
        flat
      >
        <v-toolbar-title>Tipos de autores</v-toolbar-title>
        <v-divider
          class="mx-4"
          inset
          vertical
 ...
 <template v-slot:item.idTipoAutor="{ item }">
      <v-chip
      v-if="item.cColor == null"
...
export default {
    ...
    data: () => ({
      ...
      headers: [
        ...
        { text: 'Tipo de autor', value: 'cTipoAutor' },
    ...
    mounted(){
       this.$can('/tiposautor', 'No tiene permisos para ver esta pantalla')
    },
    created(){
        this.initialize()
    },
    ...
    computed:{
      ...
      dlgTitle () {
        return this.delItem === 1 ? '¿Esta seguro de dar de baja este tipo de autor?' : '¿Esta seguro de dar de alta este tipo de autor?'
      },
      dlgSubTitle () {
